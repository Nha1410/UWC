function BoxItem({ iconComponent, title, numberNotifications, textColor, isActive }) {
    ...
    return (
        <div
            className={`w-full ${textColor} text-lg flex py-[5px] justify-center items-center hover:cursor-pointer hover:bg-[#99B2C6] hover:bg-opacity-20`}
        >
            <div className={`pl-[30px] py-[10px] ${classNameIsActive}`}>{iconComponent}</div>
            <div className="flex-1 px-[15px] py-[10px]">{title}</div>
            {numberNotifications && (
